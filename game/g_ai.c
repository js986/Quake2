...
void HuntTarget (edict_t *self)
{
	...
	//if (self->enemy == NULL && strcmp(self->classname, "zurkon") == 0)
		//gi.dprintf("No enemy for Mr. Zurkon");
...
edict_t * zurkon_find_enemy(edict_t *self)
{
	edict_t *ent = NULL;
	edict_t *best = NULL;
	while (ent = findradius(ent, self->s.origin, 500) && ent != NULL)
	{
		if (ent != self && ent->takedamage && ent->health && ent->health > 1 && !ent->client){
			best = ent;
			return best;
		}
	}
	/*
	edict_t* ent = NULL;
	while (ent = findradius(ent, self->s.origin, 3000.0f)){
		if (ent != self && !ent->client && !ent->takedamage)

			//return ent;
	}
	gi.dprintf("No hostile entity found");
	ent = NULL;
	return ent;
	*/
}
